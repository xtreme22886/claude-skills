...
- `git`, `bash`, `python3` (≥3.10), `pip`, `curl`
...
# install Python deps for the adapters
pip install --user -r adapter/requirements.txt
...
2. Symlinks `SKILL.md` → `~/.claude/skills/dev-team/SKILL.md`.
3. Symlinks each `commands/*.md` → `~/.claude/commands/<name>.md`.
4. Symlinks `hooks/statusline.sh` → `~/.claude/statusline.sh` (and ensures executable bit).
5. Adds (idempotently) the relevant entries to `~/.claude/settings.json`:
...
6. Drops a marker file at `~/.claude/.dev-team-installed` recording the install path and version.
...
**`pip install` fails with permission errors** — make sure you're using `--user` (it's in the install command above), or use a virtualenv if your distro restricts user-site installs.
